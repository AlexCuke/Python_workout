def get_final_line(filename):
    """вывод последней строки. функцию (get_fi nal_line),
которая принимает в качестве аргумента имя файла. Функция
должна печатать на экран последнюю строку этого файла."""
...
def sum_in_file(filename):
    """Сумма перемноженных чисел в строке.Создайте текстовый файл (с помощью редактора, не обяза-
тельно Python), содержащий два столбца, разделенных та-
буляцией, каждый из которых содержит число. Затем с по-
мощью Python прочитайте созданный файл. Для каждой
строки умножьте каждое первое число на второе, а затем
просуммируйте результаты всех строк. Игнорируйте все
строки, которые не содержат двух числовых столбцов"""
    total_sum = 0
